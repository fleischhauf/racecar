...
public bool OOQQOQCQOD;
public bool ODQDOQOO;
public float OQODODCQCQ;
public float ODOOQQOO;
public Transform[] OODQDQDQQQs;
public float[] trperc;
...
public string OOCQDCDOQD = "0";
public string ODOCDDQQQC = "0";
...
if(!objectScript.OCDCCCOCOC){  
Vector3 change = transform.position - oldPos;
if(OOQQOQCQOD && oldPos != Vector3.zero && change != Vector3.zero){
int i = 0;
foreach(Transform tr in OODQDQDQQQs){
...
changed = true;
if(objectScript.OCDCCCOCOC){
objectScript.OOCQDCOCCQ.specialRoadMaterial = true;
}
}
...
for(int i = 0; i < objectScript.OODQDQDQQQs.Length; i++){
if(objectScript.OODQDQDQQQs[i] == this.gameObject)return true;
}
return false;
}
}
